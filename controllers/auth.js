...
const register = async(req,res) => {    
    try {
        ...
        if(user){
            return res.status(500).json({message:"Bu email hesabi zaten bulunmaktadir."})
        }

        if(password.length < 6){
            return res.status(500).json({message:"Parolaniz 6 karakterden kücük olmamali"})    
        }
...
const login = async(req, res) => {
    try {
        ...
        if(!user){
            return res.status(500).json({message:"Böyle bir kullanici bulunamadi"})
        }
